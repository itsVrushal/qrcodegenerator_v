...
export default function Contact({ theme }) {
  ...
  return (
    <section id="contact" className={`relative py-24 transition-colors duration-700 ${
        isDark ? "bg-slate-950" : "bg-slate-50"
    }`}>
      <div className={`absolute top-0 left-0 right-0 h-px bg-gradient-to-r via-opacity-50 to-transparent ${
          isDark ? "from-transparent via-blue-500/50" : "from-transparent via-amber-500/30"
      }`}></div>
...
function ContactCard({ theme, icon, title, content }) {
  const isDark = theme === "dark";
  return (
    <div className={`flex flex-col items-center p-8 rounded-2xl border transition-all duration-300 ${
        isDark 
        ? "bg-slate-900 border-white/5 hover:border-blue-500/30" 
        : "bg-white border-slate-200 hover:border-amber-400/50 shadow-sm"
    }`}>
      <span className={`text-4xl mb-4 p-4 rounded-full ring-1 transition-colors ${
