...
export default function Contact({ theme }: ContactProps) {
  ...
  return (
    ...
    >
      <div
        className={`absolute top-0 left-0 right-0 h-px bg-gradient-to-r via-opacity-50 to-transparent ${
          isDark
            ? "from-transparent via-blue-500/50"
            : "from-transparent via-amber-500/30"
        }`}
      ></div>
      ...
      <div className="max-w-4xl mx-auto px-6 text-center">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <ContactCard
            ...
            icon="📞"
            title="24/7 Support"
            content="+91 73500 75300"
          />
          <ContactCard
            theme={theme}
            icon="✉️"
            title="Email Us"
            content="mosaicendeavours@gmail.com"
          />
        </div>
      </div>
    </section>
...
function ContactCard({ theme, icon, title, content }: ContactCardProps) {
  ...
  return (
    <div
      className={`flex flex-col items-center p-8 rounded-2xl border transition-all duration-300 ${
        isDark
    ...
    >
      <span
        className={`text-4xl mb-4 p-4 rounded-full ring-1 transition-colors ${
          isDark
            ? "bg-blue-500/10 ring-white/10"
